...
int main() {
   ...
   // ------------------ WRITE CODE HERE ------------------
    ...
    child_pid = clone(
        child_function,
        child_stack + CHILD_STACK_SIZE, CLONE_NEWUTS | CLONE_NEWPID | SIGCHLD,
        pipefd
    );

    if (child_pid == -1)
        errExit("failed clone");
    ...
    */
    ...
    // ------------------ WRITE CODE HERE ------------------
    int child_pid_fd = pidfd_open(child_pid, 0);
    
    // Changing the PID namespace of parent so that further children will be created in the
    // specified PID namespace
    setns(child_pid_fd, CLONE_NEWPID);
    ...
    if (fork() == 0) {
        ...
        */
        ...
        // Setting the UTS namespace here
        setns(child_pid_fd, CLONE_NEWUTS);
