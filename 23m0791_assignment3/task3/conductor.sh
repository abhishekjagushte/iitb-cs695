...
run()
{
    ...
    unshare -ufpnmi --kill-child  chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "
    mount -t sysfs none /sys && chmod 755 / && mount -t proc none /proc &&
    exec $INIT_CMD_ARGS" 
...
exec()
{
    ...
    nsenter -t $CONTAINER_INIT_PID -u -p -n -m -i -w -r  /bin/bash -c "exec $EXEC_CMD_ARGS"
}
...
addnetwork()
{
    ...
    ip link set dev "$INSIDE_PEER" netns "$NAME"
